...
for mcid = 1:MC
    ...
    xhat(2*mcid-1:2*mcid,1) = sum(xpart_prior.*repmat(wgt,2,1),2);
    sgmhat(2*mcid-1:2*mcid,1) = [sqrtP0(1,1);sqrtP0(2,2)];
    ESS(mcid,1) = 1/sum(wgt.^2, 2);
    for i = 2:simLen
        if ESS(mcid,i-1) < ESS_thres
        ...
        xhat(2*mcid-1:2*mcid,i) = sum(xpart_prior.*repmat(wgt,2,1),2);
        P = (xpart_prior - repmat(xhat(2*mcid-1:2*mcid,i),1,N))*diag(wgt)*...
            (xpart_prior - repmat(xhat(2*mcid-1:2*mcid,i),1,N)).';
        sgmhat(2*mcid-1,i) = sqrt(P(1,1));
        sgmhat(2*mcid,i) = sqrt(P(2,2));
        ESS(mcid,i) = 1/sum(wgt.^2, 2);
    end
...
%% Data acquisition
save 1_f_PF_SIR_4_4.mat x_true xhat ESS