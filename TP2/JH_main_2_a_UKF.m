...
for mcid = 1:MC
    tic;
    [xhat(mcid, 1), Phat, X_s, w] = ukf_predict3(x_0,P_0,f_func,u_n,v_n,1);
    [xhat(mcid, 1), Phat] = ukf_update3(xhat(mcid,i),Phat,Y(mcid,1),h_func,v_n,X_s,w,[]);
    sgmhat(mcid,1) = sqrt(Phat(1,1));
    for i = 2:simLen
        [xhat(mcid,i), Phat, X_s, w] = ukf_predict3(xhat(mcid, i-1),Phat,f_func,u_n,v_n,i);
        [xhat(mcid,i), Phat] = ukf_update3(xhat(mcid,i),Phat,Y(mcid,i),h_func,v_n,X_s,w,[]);
...
%% Data acquisition
save 2_a_UKF.mat X Y Y_r xhat sgmhat time_spent